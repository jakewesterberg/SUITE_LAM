function [psd_out, psd_norm, f] = PSD(lfp_in, varargin)
...
[msg,nfft,Fs,window,noverlap,p,dflag]=PSD_CHECK(varargin,lfp_in(1,:));
error(msg)
...
for i_f = 1 : size(psd_out, 2)
    
    for i_ch = 1 : size(lfp_t, 1)
        
        psd_norm( i_ch, i_f ) = ( psd_out( i_ch, i_f ) ...
            - mean( psd_out( :, i_f ) ) ) ...
            / mean( psd_out( :, i_f ) ) * 100;
